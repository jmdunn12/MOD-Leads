...
- **City spelled wrong on the lead sheet** — when the typo shows up in the "Cities with no coordinates" box, use the **fix spelling** dropdown on that row and pick the real city. From then on that spelling is corrected automatically: the board and the .xlsx show the right city, the drive time works, and the Assignment Flag notes `city spelling corrected from "Balitmore"` so nothing is hidden. Manage them anytime in Setup → **City spelling fixes** (add one directly there too, e.g. `balitmore|md` → Baltimore, MD). Don't use this for a genuinely new town — give that one coordinates instead.
- **New rep (hired)** — Setup → **Manage reps** → type their name, pick their team (Beta/Delta = DC Metro, Gamma-VAB = Hampton Roads, Gamma-RVA = Richmond, Kappa = bath crew), enter their home as `city|state`, click **Add rep**. They appear in their market's roster immediately and score off the team average (or their 60-day numbers) until the power ranking catches up — once they show up in a re-uploaded ranking file, the real data takes over automatically and you can delete the manual entry.
- **Rep left the company** — Setup → **Manage reps** → pick them under "Hide rep". They disappear from every roster and saved preset instantly, even while old reference files still list them. Restorable anytime (also handy for leaves of absence).
- **Rep on the ranking but not the phone list** — Setup → **Home overrides** still covers that: give them a home as `city|state` (e.g. `crofton|md`) and they appear.
